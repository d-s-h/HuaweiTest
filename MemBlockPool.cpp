...
uint8_t* MemBlockPool::acquireMemBlock(uint8_t ownerId, int limit)
{
  LOG("(%u) ->MemBlockPool::acquireMemBlock: owner %d\n", getCurrentThreadId(), ownerId);
  assert(ownerId > 0);
  ...
  LOG("(%u) <-MemBlockPool::acquireMemBlock 0x%p\n", getCurrentThreadId(), block);
  return block;
}
...
void MemBlockPool::releaseMemBlock(const uint8_t* block)
{
  ...
  LOG("(%u) ->MemBlockPool::releaseMemBlock: idx %lld owner %d 0x%p\n", getCurrentThreadId(), idx, mFreeBufferBlocks[idx], block);
